...
describe('Session 3: Branch Rename Infrastructure', () => {
  ...
  describe('CITY_NAMES export', () => {
    test('city-names.ts exports CITY_NAMES array', () => {
      const cityNamesPath = path.join(
        __dirname,
        '..',
        '..',
        '..',
      ...
      const content = fs.readFileSync(cityNamesPath, 'utf-8')
      expect(content).toContain('export const CITY_NAMES')
    })
  })
})
