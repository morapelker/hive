...
// Import the pure function and city names after mocks are in place
import { canonicalizeBranchName } from '../../../src/main/services/git-service'
import { CITY_NAMES } from '../../../src/main/services/city-names'
...
describe('Session 4: Auto-Rename Branch on First Title', () => {
  describe('auto-rename logic in opencode-service', () => {
    test('session.updated handler includes auto-rename branch logic', () => {
      ...
      expect(content).toContain('Auto-rename branch if still a city name')
      expect(content).toContain('getWorktreeBySessionId')
      expect(content).toContain('CITY_NAMES.some')
      expect(content).toContain('canonicalizeBranchName')
      expect(content).toContain("'worktree:branchRenamed'")
    })

    test('imports CITY_NAMES and canonicalizeBranchName', () => {
      const servicePath = path.join(
        __dirname,
        '..',
        '..',
      ...
      expect(content).toContain("import { CITY_NAMES } from './city-names'")
    ...
    test('sets branch_renamed=1 on failure to stop retrying', () => {
      ...
      const autoRenameBlock = content.slice(
        content.indexOf('Auto-rename branch if still a city name'),
  ...
  describe('city name detection logic', () => {
    test('CITY_NAMES contains expected cities', () => {
      expect(CITY_NAMES).toContain('tokyo')
      expect(CITY_NAMES).toContain('oslo')
      expect(CITY_NAMES).toContain('lima')
    })

    test('city name matching is case-insensitive', () => {
      const branch = 'Tokyo'
      const isCityName = CITY_NAMES.some((city) => city.toLowerCase() === branch.toLowerCase())
      expect(isCityName).toBe(true)
    })

    test('non-city branch names are not matched', () => {
