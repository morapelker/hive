...
### 🤖 **내장 AI 코딩 세션**
**OpenCode**와 **Claude Code**를 모두 지원하여 Hive 내에서 직접 AI 코딩 에이전트를 실행. 실시간으로 응답을 스트리밍하고, 도구 호출 실행을 관찰하며, 필요에 따라 권한을 승인. 완전한 undo/redo 지원으로 항상 제어할 수 있습니다.
...
### 3️⃣ **AI로 코딩 시작**
Worktree 열기 → **"New Session"** 클릭 → OpenCode 또는 Claude로 코딩 시작
...
- **커넥션 강화** — 다자간 연결, 연결 그룹, 스마트 제안
- **클라우드 동기화** — 설정, 세션, 연결 템플릿을 기기 간 동기화
- **팀 기능** — Worktree를 공유하고 실시간으로 협업
- **커스텀 AI 모델** — 자체 OpenAI/Anthropic API 키 사용
- **터미널 탭** — Worktree당 여러 터미널 세션
- **Git 그래프 시각화** — 비주얼 브랜치 히스토리와 머지
- **성능 프로파일링** — 최적화를 위한 내장 도구

### 🎯 미래 비전

- **커넥션 인텔리전스** — git 히스토리 기반 AI 연결 제안
- **커넥션 마켓플레이스** — 연결 템플릿 공유 및 발견
...
│   ├── ipc/               # IPC 핸들러 모듈
│   └── services/          # Git, OpenCode, 로거, 파일 서비스
