 ...
 * CLI-agent ticket (claude-cli or grok-cli — or one with no session yet, so it
 * can be pre-armed) whose plan is not already awaiting approval. Goal-mode
 * tickets keep their own plan→implementor handoff flow and are excluded.
 */
