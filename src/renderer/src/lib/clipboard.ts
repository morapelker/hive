import { unwrapEnvelope } from './ipc-envelope'
 ...
 *  2. Electron's main-process clipboard via IPC — reliable inside the desktop
 *     app even when the renderer's Clipboard API is blocked. This mirrors the
 *     paste path in XtermBackend, which already falls back to
 *     `window.projectOps.readFromClipboard` for the same reason.
...
export async function copyTextToClipboard(text: string): Promise<boolean> {
  ...
  try {
    if (window.projectOps?.copyToClipboard) {
      unwrapEnvelope(await window.projectOps.copyToClipboard(text))
      return true
    }
  } catch {
    // IPC unavailable or main-process write failed — fall through.
  }
