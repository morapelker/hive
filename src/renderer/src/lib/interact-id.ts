/**
 * InteractId — mirror of the encoder in hive-enterprise (`src/lib/interact-id.ts`).
 *
 * Every id is `{ulid}-{type}` internally; over GraphQL it travels as
 * `{base58(ulid)}_{type}`. The trailing type suffix is mandatory. The
 * hive-enterprise scalar decodes inputs back to the internal form, so this
 * client must send the external form. `base58` is the case-sensitive Bitcoin
 * alphabet; the ULID is treated as a 128-bit integer and round-trips exactly.
 *
 * Kept dependency-free (plain Errors, no `graphql`) so it is safe in the
 * renderer bundle.
 */
