...
import { storageApi, type CompactionPreview, type StorageStats } from '@/api/storage-api'
import {
  AlertDialog,
  AlertDialogCancel,
  AlertDialogContent,
...
export function SettingsStorage(): React.JSX.Element {
  ...
  const loadStats = async (): Promise<void> => {
    ...
    try {
      setStats(await storageApi.getStats())
    } catch (error) {
  ...
  const handleAnalyze = async (): Promise<void> => {
    setAnalyzing(true)
    try {
      const nextPreview = await storageApi.previewCompaction()
  ...
  const handleCompact = async (): Promise<void> => {
    setCompacting(true)
    try {
      const result = await storageApi.compact()
      toast.success(
