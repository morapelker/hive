...
import { isPerfProbeEnabled } from '@/lib/perf-marks'
...
export function useVimNavigation(): void {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent): void => {
      const shouldProbe = isPerfProbeEnabled()
      const t0 = shouldProbe ? performance.now() : 0
      let path = 'unhandled'
      ...
      try {
        ...
        if (event.key === 'Escape') {
          if (hint.mode === 'pending') {
            ...
            path = 'escape-pending-hint'
            return
          }
          if (vim.mode === 'insert') {
            vim.enterNormalMode()
            event.preventDefault()
            path = 'escape-insert-mode'
            return
          }
          if (vim.helpOverlayOpen) {
            vim.setHelpOverlayOpen(false)
            event.preventDefault()
            path = 'escape-help-overlay'
            return
          }
          path = 'escape'
          return
        ...
        if (shouldProbe) {
          ...
          if (duration > 1) {
            console.debug('[useVimNavigation] keydown', {
              key: event.key,
              path,
