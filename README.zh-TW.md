...
### 🤖 **內建 AI 編碼工作階段**
在 Hive 中直接執行 AI 編碼代理，支援 **OpenCode** 和 **Claude Code**。即時串流回應、觀察工具呼叫執行、按需核准權限。完整的復原/重做支援讓你始終掌握控制權。
...
### 3️⃣ **用 AI 開始編碼**
開啟一個 worktree → 點擊 **「New Session」** → 使用 OpenCode 或 Claude 開始編碼
...
- **自訂 AI 模型** — 使用自己的 OpenAI/Anthropic API 金鑰
- **終端機標籤頁** — 每個 worktree 多個終端機工作階段
- **Git 圖形視覺化** — 視覺化分支歷史和合併
- **效能分析** — 內建最佳化工具

### 🎯 未來願景

- **連接智慧** — 基於 git 歷史的 AI 驅動連接建議
- **連接市集** — 分享和探索連接範本
- **遠端開發** — 基於 SSH 和容器的開發
- **三方連接** — 視覺化連接和合併多個分支
